...
# ---------------------------------------------------------------------------
# VENDORED from RLinf (examples/recap/process/compute_returns.py),
# https://github.com/RLinf/RLinf — RECAP Stage 1. Copied verbatim; it is
# self-contained (no rlinf/openpi imports). Kept here so limb's RECAP dataset
# can be turned into a returns sidecar without a full RLinf checkout.
#
# INPUT SCHEMA (read from each data/**/*.parquet):
#   episode_index, frame_index  — present in limb convert-lerobot output
#   is_success                  — PER-FRAME bool; only the LAST frame of each
#                                 episode is read (`is_success_col[ep_end-1]`).
#                                 REQUIRED when dataset_type="rollout".
#   task OR task_index          — for the prompt sidecar column
# See limb/docs/recap_collection.md § "What's left to do" Task 1.
# ---------------------------------------------------------------------------
