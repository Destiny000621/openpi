...
class FrankaInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        if self.state_dim in (10, 17):
            # rot6d state (10-D, or 17-D with appended joint_pos): dims 3:9 are
            # rotation-matrix entries, mathematically in [-1, 1] but float rounding
            # can nudge past ±1. The pi05 discrete-state tokenizer sends values
            # strictly below -1 to bin "-1" (np.digitize underflow), so clamp the
            # eps excursions. No-op for in-range values.
